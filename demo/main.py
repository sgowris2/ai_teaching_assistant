    ...
    try:
        ...
        try:
            for image_dict in images:
                ...
                saved_filepath = 'uploaded_images/w{}-s{}-{}.{}'.format(worksheet_id, student_id, datetime.now().strftime('%Y%m%d%H%M'), image_dict['name'].split('.')[-1])
...
def autograde_worksheet():
    ...
    images = [{'name': x['name'], 'type': x['type'], 'binary': io.BytesIO(base64.b64decode(x['data'])).getvalue()} for x in images_data]
